import { RendererFactory, RenderShapeTypes } from './halftone-renderer';
...
export class BaseHalftoneElement extends HTMLElement {
    ...
    resize() {
        ...
        this.renderer.displayWidth = this.visibleRect.width;
        this.renderer.displayHeight = this.visibleRect.height;
        return true;
    };
        ...
        switch (name) {
            ...
            case 'inverse':
                if (this.hasAttribute('inverse')) {
                    this.renderer.inverse = true;
                } else {
                    this.renderer.inverse = false;
                }
                this.render();
                return;

            case 'inputsizeconstraint':
                this.renderer.inputSizeConstraint = newValue;
                this.render();
                return;
    ...
    async startCamera(opts) {
        ...
        if (opts && opts.facingMode) {
            constraints.video.facingMode = opts.facingMode;
        }
    ...
    createRendererOptions() {
        ...
        if (this.hasAttribute('normalizedistance')) {
            opts.normalizeDistanceBetween = true;
        }
        if (this.hasAttribute('inputsizeconstraint')) {
            opts.inputSizeConstraint = Number(this.getAttribute('inputsizeconstraint'));
        }
        if (this.hasAttribute('inverse')) {
            opts.inverse = true;
        }
