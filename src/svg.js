...
export class HalftoneSVG extends BaseHalftoneElement {
     ...
     * @return string
     */
    getSVG(width, height, color) {
        const clr = color ? `style="background-color: ${color}"` : '';
        return `<svg ${clr} width="${width | this.visibleRect.width}" height="${height | this.visibleRect.height}" xmlns="http://www.w3.org/2000/svg">
    ...
    get svgPathWithTransformGroup() {
        if (!this.visibleRect.width || !this.renderer.width) {
            return ``;
        }
